...
function sellTickets(queue) {
  let sellerHasMoney = true;
  queue.reduce((cashForChange, bill) => {
    if (bill === 25) {
      return cashForChange + bill;
    }
    if (cashForChange < bill - 25) {
      sellerHasMoney = false;
    }
    return cashForChange + 25;
  }, 0);
  return sellerHasMoney;
}
